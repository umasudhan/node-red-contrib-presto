...
var Client = exports.Client = function(args){
  ...
  this.userAgent = 'node-red-presto-client ' + exports.version;
...
Client.prototype.request = function(opts, callback) {
  ...
  if (opts instanceof Object) {
    ...
    if (client.basic_auth)
      opts.headers[Headers.AUTHORIZATION] = 'Basic ' + Buffer.from(client.basic_auth.user + ":" + client.basic_auth.password).toString("base64");
    ...
    if (client.basic_auth){
      ...
      opts.headers = {};
      opts.headers[Headers.AUTHORIZATION] = 'Basic ' + new Buffer.from(client.basic_auth.user + ":" + client.basic_auth.password).toString("base64");

      opts.agent = new adapter.Agent(opts); // Otherwise SSL params are ignored.
    }

    opts = Object.assign({}, opts, client.ssl); // make sure we always reapply the ssl options
